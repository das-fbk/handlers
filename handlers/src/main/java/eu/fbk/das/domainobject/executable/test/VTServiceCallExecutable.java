...
public class VTServiceCallExecutable extends
	...
	private ArrayList<Long> serviceExecTime = new ArrayList<Long>();
	...
	public void execute(ProcessDiagram proc, ProcessActivity pa) {
		...
		if (doiState != null) {
			...
			viaggiaAlternatives = new ArrayList<TravelViaggiaTrento>();
			...
			if (user != null) {
				pe.addViaggiaTrentoAlternatives(user.getId(),
						viaggiaAlternatives);
			}

			long totalAlt = 0;
			if (viaggiaAlternatives != null && viaggiaAlternatives.size() != 0) {
				for (TravelViaggiaTrento alt : viaggiaAlternatives) {
					totalAlt += alt.getRoutes().size();
				}
			}

			pe.setTestServicesLog(user.getId(), "ViaggiaTrento",
					serviceExecTime.get(0), viaggiaAlternatives.size(),
					totalAlt / viaggiaAlternatives.size());
